...
mol = tq.Molecule(geometry="h 0.0 0.0 -1.5\nBe 0.0 0.0 0.0\nh 0.0 0.0 1.5", basis_set="sto-3g", active_orbitals=[1,2,5,6])
# c,h,g = mol.get_integrals()
# mol = tq.Molecule(geometry=mol.parameters.geometry, nuclear_repulsion=c, one_body_integrals=h, two_body_integrals=g.elems, basis_set=mol.parameters.basis_set)
fci = mol.compute_energy("fci")
print(f"fci: {fci}")
H = mol.make_hamiltonian()

# mol.print_basis_info()
# print(mol.integral_manager)

# hmol = sun.Molecule(geometry="h 0.0 0.0 -1.5\nBe 0.0 0.0 0.0\nh 0.0 0.0 1.5", basis_set="sto-3g").use_native_orbitals()
...
# print(f"error: {fci - wfn.inner(H * wfn).real}") # check
