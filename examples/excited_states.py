...
v,vv = scipy.sparse.linalg.eigsh(Hsparse, k=50, sigma=fci)
# print(f"error: {fci - v[0]}") # check

exc_counter = []
for i in range(vv.shape[1]):
    wfn = tq.QubitWaveFunction.from_array(vv[:,i])
    s2 = mol.make_s2_op()
    sz = mol.make_sz_op()
    U = tq.gates.Rz(angle=0, target=range(H.n_qubits)) # dummy circuit
    if np.isclose(tq.simulate(tq.ExpectationValue(U,s2), initial_state=wfn),0):
        energy = wfn.inner(H * wfn).real
        assert np.isclose(v[i] - wfn.inner(H * wfn).real, 0), "eigenvalue is different from expectation value"
        exc_counter.append(i)

print(exc_counter)

orig_mol = mol
all_errors = []
for i in exc_counter:
    print(f"i: {i}")
    ...
    # Create rotators
    URs, variables, blocks = mvb.initialize_rotators(mol, graphs, solve_GNM=False, fix_gnm_values=True, add_relax=False, silent=True)
    variables = {}
    for block in blocks:
    ...
    if debug:
        ...
        H1 = mvb.get_one_body_operator(mol)
        H2 = mvb.get_two_body_operator(mol)
        E1 = tq.ExpectationValue(U=U, H=H1)
        E2 = tq.ExpectationValue(U=U, H=H2)
        print(energy - tq.simulate(E1 + E2, initial_state=wfn))
        print(f"energy == E1+E2: {np.isclose(energy,tq.simulate(E1 + E2, initial_state=wfn))}")
        print("done")
    ...
    if debug:
        ...
        # print("TEST 4")
        # E_test = tq.ExpectationValue(U=U, H=mol.make_hamiltonian())
        # energy_test = tq.simulate(E_test, initial_state=wfn)
        # print(f"energy_test: {energy_test}")
        # tmol = fold_rotators(mol, blocks[0].get_UR_circuit().map_variables(variables))
    ...
    M_tot = mvb.compute_num_meas(wfn, is_hcb=True, hcb_mol=hcb_mol, debug=False, n_repetitions=100)
    ...
    for i,rotation in enumerate(new_rotations):
        _, mol, approx, _ = mvb.rotate_and_hcb(UR=rotation.get_UR_circuit(), circuit=U,
                                        ...
                                        old_UR=old_UR, debug=debug, initial_state=wfn, true_wfn=wfn, n_repetitions=100)
